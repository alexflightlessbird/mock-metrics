...
import { useStudentDetails, useStudentTeams } from "../common/hooks/useStudentDetails";
import { useSchoolTeams } from "../common/hooks/useSchoolDetails";
import BasePage from "../common/components/BasePage";
import { Button, Divider, Flex, Group, Select, Stack, Text } from "@mantine/core";
...
export default function StudentInfoPage() {
    ...
    const { data: studentTeams, isLoading: studentTeamsLoading = true, removeTeamFromStudent, addTeamToStudent } = useStudentTeams(studentId);
    const { data: allTeams, isLoading: allTeamsLoading = true } = useSchoolTeams(selectedSchoolId);
    ...
    const availableTeams = useMemo(() => {
        return allTeams?.filter((t) => t.is_active).map((t) => ({
            value: t.id,
            label: `${t.name} (${t.year}${t.type === "pre-stack" ? " Pre-Stack" : t.type === "post-stack" ? " Post-Stack" : ""})`
        }));
    }, [allTeams]);
        ...
        if (currentTeamId !== initialCurrentTeam?.team_id) {
            if (currentTeamId === null) {
                if (initialCurrentTeam !== null) {
                    ...
                    await removeTeamFromStudent({ studentId, teamId: initialCurrentTeam?.team_id });
                } else {
        ...
        >
            ...
            <Group justify="space-between" align="flex-start" mb="xs">
                <Stack gap="0">
                    ...
                    {!editMode && (
                        <Text fz="sm">{initialCurrentTeam?.teams?.name || "Not currently assigned to a team"}</Text>
                    )}
                </Stack>
                    ...
                    <PageSection title="danger zone">
                        <Flex gap="xl" align="center">
                            ...
                            <DeleteConfirmationModal
                                ...
                                includeBallots={true}
                                onSubmit={() => {
                                    deleteStudent({ studentId: selectedStudent.id, schoolId: selectedSchoolId });
                                    navigate("/school");
                                }}
                                entity={{
