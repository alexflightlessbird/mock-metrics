export default function ballotAverage(scores) {
    ...
    const pTotal = scores.filter(s => s.score_type.startsWith("p")).reduce((sum, s) => sum + s.score_value, 0);
    const dTotal = scores.filter(s => s.score_type.startsWith("d")).reduce((sum, s) => sum + s.score_value, 0);
