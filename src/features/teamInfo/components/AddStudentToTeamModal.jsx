...
export default function AddStudentToTeamModal({ onClose, trigger, teamId, teamName, existingStudentIds = [], allStudents = [] }) {
    ...
    const availableStudentsOptions = useMemo(() => {
        return availableStudents.map((student) => ({
            value: student.id,
            label: `${student.name}${currentTeamOfStudent(student.id) ? ` (${currentTeamOfStudent(student.id)})` : ""}`,
        }))
    });
    ...
    const pages = [
        ...
        <Stack key={0}>
            <Text size="sm" c="dimmed">
                This will remove the selected student from their current team if they are already assigned to one. Ballots will not be affected.
            </Text>
