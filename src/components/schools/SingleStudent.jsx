...
export default function SingleStudent({ selectedSchool, selectedStudent, allTeams, allStudentTeams, triggerReload }) {
    const schoolItem = (
        <Link to={`/schools?id=${selectedSchool.schools.id}`}>{selectedSchool.schools.name}</Link>
    );
    ...
    return (
        <>
            ...
            <Flex style={{ alignItems: "center", gap: "7px" }}>
                ...
                {(selectedSchool.role === ROLES.PRIMARY || selectedSchool.role === ROLES.ADMIN) && <EditIcon onClick={editStudentModal} />}
            </Flex>
