...
export default function SchoolTabs({ role, allUsers, allTeams, allStudents, allTournaments, triggerReload, isPremium, schoolId, schoolName, currentTab, setCurrentTab }) {
    ...
    return (
        <Tabs defaultValue={currentTab} onChange={setCurrentTab}>
            ...
            {role === ROLES.PRIMARY && (<Tabs.Panel value="users">
                <>
                    <br />
                    <Text>To add additional users, please contact MSU Mock Trial.</Text>
                    <h3>Primary Admins {isPremium ? "" : <Tooltip {...tooltipProps}><span>({primaryAdminUsers.length}/{PREMIUM_LIMITS.PRIMARY})</span></Tooltip>}</h3>
                    <UserList users={primaryAdminUsers} {...userListProps} />
                    <h3>Admins {isPremium ? "" : <Tooltip {...tooltipProps}><span>({adminUsers.length}/{PREMIUM_LIMITS.ADMIN})</span></Tooltip>}</h3>
                    <UserList users={adminUsers} {...userListProps} />
                    <h3>Viewers {isPremium ? "" : <Tooltip {...tooltipProps}><span>({viewerUsers.length}/{PREMIUM_LIMITS.VIEWER})</span></Tooltip>}</h3>
