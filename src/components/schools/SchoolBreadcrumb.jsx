...
export default function SchoolBreadcrumb({ selectedSchool, allStudents, allTeams, allTournaments }) {
    ...
    const getBreadcrumbItems = () => {
        ...
        items.push({
            title: selectedSchool || teamId || studentId || tournamentId
                ? <Link to="/schools"><Pill {...inactiveProps}>Schools</Pill></Link>
                : <Pill {...activeProps}>Schools</Pill>
        });

        if (selectedSchool) {
            items.push({
                title: teamId || studentId || tournamentId
                    ? <Link to={`/schools?schoolId=${selectedSchool.schools.id}`}><Pill {...inactiveProps}>{selectedSchool.schools.name}</Pill></Link>
